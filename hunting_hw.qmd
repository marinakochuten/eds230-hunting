...
We are defining stability as maintaining predator populations above 200 and prey populations above 50 after 50 years. I chose these values to allow the population to grow from 1 over time, and because it is a reasonable ratio of predators to prey that also allows the species' to be maintained over time. 
...
# Explore hunting by running model across a range of values

##### rhunt = 0.01 #####
pars <- c(base_params, rhunt = 0.01, prey_min = 100)
...
ggplot(res_long, aes(x = time, y = population, color = species)) +
    geom_line(size = 1) +
    labs(
        title = "Predator and Prey Populations Over Time with rhunt = 0.01",
        x = "Time (days)",
        y = "Population",
        color = "Species"
    ) +
...
```{r}
##### rhunt = 0.05 #####
pars <- c(base_params, rhunt = 0.05, prey_min = 100)
currpop <- c(prey = 500, pred = 10)
days <- seq(from = 1, to = 500)
res <- ode(y = currpop, times = days, func = lotv_mod_hunting, parms = pars)
res_df <- as.data.frame(res)
...
ggplot(res_long, aes(x = time, y = population, color = species)) +
    geom_line(size = 1) +
    labs(
        title = "Predator and Prey Populations Over Time with rhunt = 0.05",
        x = "Time (days)",
        y = "Population",
        color = "Species"
    ) +
...
```{r}
##### rhunt = 0.04 #####
pars <- c(base_params, rhunt = 0.04, prey_min = 100)
currpop <- c(prey = 500, pred = 10)
days <- seq(from = 1, to = 500)
res <- ode(y = currpop, times = days, func = lotv_mod_hunting, parms = pars)
res_df <- as.data.frame(res)
...
ggplot(res_long, aes(x = time, y = population, color = species)) +
    geom_line(size = 1) +
    labs(
        title = "Predator and Prey Populations Over Time with rhunt = 0.04",
        x = "Time (days)",
        y = "Population",
        color = "Species"
    ) +
    theme_minimal()
```
The threshold for hunting to support a stable population is 0.048.
